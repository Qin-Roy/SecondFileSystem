...
void Inode::ReadI()
{
	...
	this->i_flag |= Inode::IACC; //内存inode被访问过，需要修改最近一次访问时间

	/* 一次一个字符块地读入所需全部数据，直至遇到文件尾 */
	while (User::my_NOERROR == u.u_error && u.u_IOParam.m_Count != 0)
	{
		lbn = bn = u.u_IOParam.m_Offset / Inode::BLOCK_SIZE; //逻辑块号
		// cout<<"lbn:"<<lbn<<endl;

		offset = u.u_IOParam.m_Offset % Inode::BLOCK_SIZE; //偏移量
		// cout<<"offset:"<<offset<<endl;

		/* 传送到用户区的字节数量，取读请求的剩余字节数与当前字符块内有效字节数较小值 */
		nbytes = (Inode::BLOCK_SIZE - offset) < /* 块内有效字节数 */u.u_IOParam.m_Count ? (Inode::BLOCK_SIZE - offset) : u.u_IOParam.m_Count;
...
void Inode::WriteI()
{
	...
	while (User::my_NOERROR == u.u_error && u.u_IOParam.m_Count != 0)
	{
		...
		/* 从用户区拷贝的字节数量，取写请求的剩余字节数与当前字符块内有效字节数较小值 */
		...
		if (u.u_error != User::my_NOERROR)	/* 写过程中出错 */
		{
...
int Inode::Bmap(int lbn)
{
	...
	*/
	...
	if (lbn >= Inode::HUGE_FILE_BLOCK) //如果逻辑块号大于最大块号数量，出错返回
	{
